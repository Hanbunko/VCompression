...
fn fold_fold_fold(selected_function: String,
    ...
    // DCTQ processes 8 rows at a time
    if selected_function == "dctq" {
        iteration_count = 720; // 5760 / 8 = 720 iterations
        if resolution == "4K" {
            iteration_count = 2160; // Will need to be adjusted for 4K DCTQ
        }
    }
    ...
    if selected_function == "hash" {
        ...
        } else if selected_function == "dctq" {
            let input_data: ZKronoInput = serde_json::from_str(&input_file_json_string).expect("Deserialization failed");
            for i in 0..iteration_count {
                let mut private_input = HashMap::new();
                // DCTQ processes 8 rows at a time
                private_input.insert("row_orig".to_string(), json!(input_data.original[(i*8)..(i*8)+8]));
                private_input.insert("row_tran".to_string(), json!(input_data.transformed[(i*8)..(i*8)+8]));
                private_inputs.push(private_input);
            }
        } else {
            let input_data: ZKronoInput = serde_json::from_str(&input_file_json_string).expect("Deserialization failed");
...
fn main() {
    let matches = App::new("VIMz")
        ...
        .arg(
            ...
            .takes_value(true)
            .possible_values(&["crop", "fixedcrop", "grayscale", "resize", "color_transform", "sharpness", "contrast", "blur", "brightness", "hash", "dctq"])
        )
        .arg(
